...
public class Dijkstra {
        ...
        for (int cuenta = 1; cuenta <= nNodos; cuenta++) 
            for (int cnt = 1; cnt <= nNodos; cnt++){ 
                if (cnt != cuenta) {
                    ...
                    System.out.println("(Ingresa 0 si la arista no existe) ");
                    ocurrioError = false;
                    try {
    ...
    private void imprimeCamino(String cam, int nod, int o) {
        System.out.print("\nCamino: ");
        if (cam.charAt(0) == '*') 
            System.out.print("No Hay camino de porque no se sabe el punto inicial " + (char) (o + 65) + " a: " + cam.charAt(cam.length()));
         else {
